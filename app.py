...
def apply_seven_conditions(df_klines, codes):
    """7 条件筛选（对给定 codes 列表）"""
    passed = []
    for code in codes:
        ...
        if vol_5day <= 0 or vol_today / vol_5day <= 1.5:
            continue
...
### 🎯 7 条件叠加
在**三策略精选**的基础上，再加 7 条件筛选：
- 非 ST + 10日涨>5% + 量比>1.5 + 涨幅3-5% + 收>MA5 + 资金流入
...
if run:
    ...
    try:
        ...
        if not df_3.empty:
            ...
            with st.spinner("🔍 应用 7 条件筛选..."):
                seven_cond_codes = apply_seven_conditions(df_sub, df_3['代码'].tolist())
                df_7 = df_3[df_3['代码'].isin(seven_cond_codes)].copy() if seven_cond_codes else pd.DataFrame()
            
            if not df_7.empty:
                st.subheader("🎯 7 条件叠加（最严筛选）")
                st.success(f"✅ {len(df_7)} 只同时通过三策略 + 7 条件（最稳健）")
                st.dataframe(df_7, use_container_width=True, hide_index=True)
            else:
                st.info("💡 三策略通过的股未通过 7 条件全部 - 可以单看三策略精选")
            st.divider()
